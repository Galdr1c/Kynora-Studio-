...
const AIAssistantPanel: React.FC<AIAssistantPanelProps> = ({ 
  ...
  return (
    <div className="fixed right-6 top-24 bottom-24 w-[380px] bg-[#1A1A1E]/95 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-in slide-in-from-right-12 duration-500 z-[100]">
      ...
      <div className="flex px-4 py-3 bg-black/20 gap-2">
        <button 
          onClick={() => setActiveTab('suggestions')}
          className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'suggestions' ? 'bg-white/10 text-white shadow-xl' : 'text-gray-500 hover:text-gray-300'}`}
        >
          Insights
        </button>
        <button 
          onClick={() => setActiveTab('competitor')}
          className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'competitor' ? 'bg-white/10 text-white shadow-xl' : 'text-gray-500 hover:text-gray-300'}`}
        >
          Competitive
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide p-6 space-y-6">
        {activeTab === 'suggestions' ? (
          <>
            {suggestions.length === 0 && !isAnalyzing && (
              <div className="flex flex-col items-center justify-center h-full text-center space-y-6 opacity-40">
                <div className="w-20 h-20 rounded-full border-2 border-dashed border-gray-600 flex items-center justify-center"><BarChart3 size={32}/></div>
                <div className="space-y-2">
                  <p className="text-xs font-bold uppercase tracking-widest">No Active Insights</p>
            ...
            {!isAnalyzing && suggestions.map((s) => (
              <div 
                key={s.id} 
                className="group p-5 bg-white/[0.03] border border-white/5 rounded-3xl hover:bg-white/[0.05] hover:border-purple-500/30 transition-all cursor-pointer relative overflow-hidden"
              ...
              >
                ...
                <h4 className="text-xs font-bold text-white mb-2 group-hover:text-purple-400 transition-colors">{s.title}</h4>
          ...
          <div className="space-y-8">
            <div className="p-8 bg-black/40 border-2 border-dashed border-white/5 rounded-[2.5rem] flex flex-col items-center text-center space-y-4">
              <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center text-gray-500"><Target size={32} /></div>
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-white uppercase tracking-widest">Market Differentiation</h4>
              ...
              <label className="w-full px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl text-[10px] font-black uppercase tracking-widest border border-white/10 transition-all cursor-pointer flex items-center justify-center space-x-3">
                <CloudUpload size={14} />
            ...
            <div className="space-y-4">
               ...
               <div className="p-6 bg-purple-600/5 border border-purple-500/10 rounded-3xl">
                  <p className="text-[10px] text-gray-400 italic leading-relaxed">"Neural analysis of the culinary tech landscape suggests a shift towards organic high-contrast minimalism. Avoid heavy gradients used by legacy players."</p>
               </div>
            </div>
          </div>
        )}
