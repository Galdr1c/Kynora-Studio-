...
const StyleTransferPanel: React.FC<StyleTransferPanelProps> = ({ currentStyles, isProcessing, onApplyStyle, onDownloadStyle }) => {
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold tracking-tight text-[#1D2B3A] dark:text-white uppercase">Artistic Neural Styles</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
          {STYLE_LIBRARY.map((style) => (
            <button
              ...
              className={`group relative p-6 rounded-[2rem] border transition-all text-left flex flex-col justify-between aspect-square ${selectedStyleId === style.id ? 'bg-purple-600 border-transparent shadow-2xl shadow-purple-600/30' : 'bg-white dark:bg-white/5 border-gray-100 dark:border-white/5 hover:border-purple-600/20'}`}
            >
              <div className="flex items-start justify-between">
                <div className={`p-2 rounded-xl ${selectedStyleId === style.id ? 'bg-white/20' : 'bg-purple-600/10'}`}>
                ...
                <div className="group/hint relative">
                  <Info size={14} className={selectedStyleId === style.id ? 'text-white/50' : 'text-gray-300 dark:text-gray-600'} />
              ...
              <div className="space-y-1">
                ...
                <h3 className={`text-xs font-black uppercase tracking-tighter ${selectedStyleId === style.id ? 'text-white' : 'text-gray-800 dark:text-gray-200'}`}>{style.label}</h3>
              </div>
            </button>
          ))}
        </div>
        ...
        <aside className="lg:col-span-4 space-y-8">
           <div className="bg-white dark:bg-[#161618] rounded-[2.5rem] border border-gray-100 dark:border-white/5 p-8 shadow-xl space-y-10">
              <div className="flex items-center space-x-4">
                 ...
                 <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-gray-800 dark:text-gray-200">Neural Config</h3>
              ...
              <button 
                ...
                className={`w-full py-5 rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center justify-center space-x-4 shadow-2xl ${!selectedStyleId || isProcessing ? 'bg-white/5 text-gray-500 cursor-not-allowed border border-white/5' : 'bg-purple-600 hover:bg-purple-500 text-white shadow-purple-600/30'}`}
              >
             ...
             <div className="space-y-6">
                ...
                <div className="space-y-4">
                   {currentStyles.map((style) => (
                     <div key={style.id} className="group relative bg-white dark:bg-[#161618] rounded-3xl border border-gray-100 dark:border-white/5 p-4 flex items-center space-x-4 shadow-lg hover:border-purple-600/30 transition-all">
                        ...
                        <div className="flex-1 min-w-0">
                           <h5 className="text-[10px] font-black text-gray-800 dark:text-gray-200 uppercase truncate">{style.label}</h5>
                        ...
                        <button 
                          ...
                          className="p-3 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-purple-600 opacity-0 group-hover:opacity-100 transition-all active:scale-90"
                        >
                          <Download size={14}/>
                        </button>
                     </div>
